...
description: |
  ...
  Phase 1: Codebase understanding (Opus subagent 1M context + Codex user interaction).
...
```
Phase 1: UNDERSTAND (Opus 1M context + Codex Lead)
  Opus subagent analyzes the codebase (1M context), Codex interacts with the user
    ↓
Phase 2: RESEARCH & DESIGN (Agent Teams — Parallel)
  Researcher (Opus) ←→ Architect (Codex) communicate bidirectionally for research and design
    ↓
Phase 3: PLAN & APPROVE (Codex Lead + User)
  Integrate research and design, create plan and get user approval
```

---

## Phase 1: UNDERSTAND (Opus Subagent + Codex Lead)

**Analyze the codebase with Opus subagent's 1M context while Codex interacts with the user.**
...
Task tool:
  ...
  prompt: |
    ...
    Save analysis to .Codex/docs/research/{feature}-codebase.md
    Return concise summary (5-7 key findings).
```

To supplement the subagent's analysis, Codex can use Glob/Grep/Read to inspect specific files.
...
1. **Researcher** — Uses WebSearch/WebFetch for external research (Opus 1M context)
   ...
   Save all findings to .Codex/docs/research/{feature}.md
   Save library docs to .Codex/docs/libraries/{library}.md
   ...
   When ALL your tasks are complete, write a work log file to:
     .Codex/logs/agent-teams/{team-name}/researcher.md
...
2. **Architect** — Uses Codex CLI for design and planning
   ...
   IMPORTANT — Work Log:
   When ALL your tasks are complete, write a work log file to:
     .Codex/logs/agent-teams/{team-name}/architect.md
...
## Phase 3: PLAN & APPROVE (Codex Lead)
...
- `.Codex/docs/research/{feature}.md` — Researcher findings
- `.Codex/docs/libraries/{library}.md` — Library documentation
- `.Codex/docs/DESIGN.md` — Architecture decisions
...
Append the project context to `AGENTS.md` **Zone C** (the area below the `@orchestra:repo-boundary` marker box) for cross-session persistence. Never touch Zone A or Zone B.
...
| File | Author | Purpose |
|------|--------|---------|
| `.Codex/docs/research/{feature}.md` | Researcher | External research findings |
| `.Codex/docs/libraries/{lib}.md` | Researcher | Library documentation |
| `.Codex/docs/DESIGN.md` | Architect | Architecture decisions |
| `AGENTS.md` (updated) | Lead | Cross-session project context |
...
- **Phase 1**: Opus subagent (1M context) analyzes the codebase while Codex interacts with the user
