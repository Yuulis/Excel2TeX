...
description: |
  ...
  Phase 1: Scope & impact analysis (Opus subagent codebase scan + Codex scope analysis + Codex user clarification).
...
```
Phase 1: SCOPE (Opus 1M context + Codex Impact Analysis + Codex Lead)
  Opus subagent scans affected areas, Codex analyzes scope and impact,
  Codex clarifies requirements with the user
    |
...
## Phase 1: SCOPE (Opus Subagent + Codex + Codex Lead)

**Understand the feature's scope and impact on the existing codebase with Opus subagent's 1M context, then consult Codex for scope and impact analysis, while Codex clarifies requirements with the user.**
...
Task tool:
  ...
  prompt: |
    ...
    Save analysis to .Codex/docs/research/add-feature-{feature}-codebase.md
...
Update `.Codex/docs/DESIGN.md` with the feature's architecture decisions:
...
### Post-Implementation: Update AGENTS.md

After implementation is complete, add feature context to AGENTS.md for cross-session persistence:
...
| File | Author | Purpose |
|------|--------|---------|
| `.Codex/docs/research/add-feature-{feature}-codebase.md` | Opus Subagent | Codebase scan for affected areas |
| `.Codex/docs/DESIGN.md` (updated) | Lead (Codex-informed) | Architecture decisions for the feature |
| `AGENTS.md` (updated) | Lead | Cross-session feature context |
...
- **Phase 1**: Opus subagent (1M context) scans affected areas, then Codex classifies scope and complexity, while Codex collects requirements from the user
