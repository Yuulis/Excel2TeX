...
description: |
  ...
  Phase 1: Error reproduction & context gathering (Opus subagent 1M context + Codex initial analysis + Codex user interaction).
...
> **Preflight:** Update CLIs before starting — `Codex update && npm install -g @openai/codex@latest`. Releases drift frequently (model names, flags, sandbox semantics).
...
```
Phase 1: REPRODUCE & UNDERSTAND (Opus 1M context + Codex Initial Analysis + Codex Lead)
...
## Phase 1: REPRODUCE & UNDERSTAND (Opus Subagent + Codex + Codex Lead)

**Reproduce the error and gather full context with Opus subagent's 1M context, then consult Codex for initial hypothesis generation, while Codex interacts with the user.**
...
Task tool:
  ...
  prompt: |
    ...
    Save analysis to .Codex/docs/research/troubleshoot-{issue}-context.md
...
1. **Root Cause Analyst** — Uses Codex CLI as PRIMARY analysis engine for deep code reasoning
   ...
   Save analysis to .Codex/docs/research/troubleshoot-{issue}-root-cause.md
   ...
   When ALL your tasks are complete, write a work log file to:
     .Codex/logs/agent-teams/{team-name}/root-cause-analyst.md
...
2. **Impact Investigator** — Uses Opus with Git history, codebase search, WebSearch, and Codex for risk analysis
   ...
   Save findings to .Codex/docs/research/troubleshoot-{issue}-impact.md
   ...
   IMPORTANT — Work Log:
   When ALL your tasks are complete, write a work log file to:
     .Codex/logs/agent-teams/{team-name}/impact-investigator.md
...
- `.Codex/docs/research/troubleshoot-{issue}-root-cause.md` -- Root cause analysis
- `.Codex/docs/research/troubleshoot-{issue}-impact.md` -- Impact assessment
...
Add bug context to AGENTS.md for cross-session persistence:
...
| File | Author | Purpose |
|------|--------|---------|
| `.Codex/docs/research/troubleshoot-{issue}-context.md` | Opus Subagent | Initial error context analysis |
| `.Codex/docs/research/troubleshoot-{issue}-root-cause.md` | Root Cause Analyst | Root cause analysis (Codex-driven) |
| `.Codex/docs/research/troubleshoot-{issue}-impact.md` | Impact Investigator | Impact assessment (with Codex risk analysis) |
| `AGENTS.md` (updated) | Lead | Cross-session bug fix context |
...
- **Phase 1**: Opus subagent (1M context) reproduces the error and gathers full context, then Codex generates initial hypotheses, while Codex collects details from the user
