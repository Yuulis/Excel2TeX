...
description: |
  ...
  from AGENTS.md Zone C, compresses the live conversation, and (optionally)
  archives old research notes. Cross-session progress lives in PROGRESS.md and the
  per-session detail in `.Codex/checkpoints/` — both are owned by /checkpointing
  and are NOT regenerated here. Run when Zone C has grown (typical trigger:
  AGENTS.md > ~400 lines or multiple stale work blocks) or as the final step of a
  /checkpointing run.
metadata:
  short-description: Compact AGENTS.md Zone C work blocks and the live conversation
disable-model-invocation: true
---

# Context Refresh

**Maintenance skill that keeps long-running working state slim: it prunes stale work blocks from AGENTS.md Zone C, compacts the live conversation, and optionally archives no-longer-active research notes.**
...
|---|---|---|
| AGENTS.md **Zone C** work blocks + the live conversation | context-refresh | **Prune & compact** |
| `PROGRESS.md` (rolling latest-5 checkpoint summaries, git-tracked) | `/checkpointing` | **Do not touch** — read-only reference |
| `.Codex/checkpoints/*.md` (per-session detail) | `/checkpointing` | **Do not touch / do not delete** |
...
- Optionally archive `.Codex/docs/research/` notes whose feature is no longer active.
- Leave PROGRESS.md, the Zone C `## Progress Tracker` link, and `.Codex/checkpoints/` exactly as `/checkpointing` left them.

## When to Use

- As the final step of a `/checkpointing` run (the conversation/Zone C "compact" step).
- AGENTS.md exceeds ~400 lines or Zone C scrolls off-screen with multiple stale work blocks.
- `.Codex/docs/research/` accumulated multiple completed-feature notes that are no longer being edited.
...
3. **Checkpoints & PROGRESS.md are off-limits**: Never delete, move, or rewrite files in `.Codex/checkpoints/`, and never regenerate `PROGRESS.md`. They are owned by `/checkpointing`.
...
- `.Codex/docs/research/archive/{feature}.md` — research notes whose feature is no longer active.
...
### AGENTS.md Zone C (only the area below `@orchestra:repo-boundary`)
...
- For each `## Current Project|Feature|Bug Fix` block: title, last update date
  (infer from git log on AGENTS.md if no inline date), 3-line gist.
...
### Research notes
- Path: `.Codex/docs/research/*.md` (NOT including `archive/`)
...
- Path: `.Codex/docs/research/archive/`
...
- `.Codex/docs/research/feat-x.md`
    -> `.Codex/docs/research/archive/feat-x.md`
...
```bash
mkdir -p .Codex/docs/research/archive
```
...
- Source: `.Codex/docs/research/{feature}.md`
- Destination: `.Codex/docs/research/archive/{feature}.md`
...
1. Run `wc -l AGENTS.md` and report new line count vs previous.
2. Confirm the `## Progress Tracker` link block is still present in Zone C.
3. List the contents of `.Codex/docs/research/` and `.Codex/docs/research/archive/`.
4. Confirm `.Codex/checkpoints/` and `PROGRESS.md` are unchanged (they were never touched).
5. Re-grep for both boundary markers to confirm they survived intact:

   ```bash
   grep -c "@orchestra:template-boundary" AGENTS.md
   grep -c "@orchestra:repo-boundary" AGENTS.md
   ```

   Both must return `1`.
6. Provide a single user-facing recap paragraph (Japanese, per `.Codex/rules/language.md`) summarising: lines removed from Zone C, work blocks retired, research notes archived, and confirmation that PROGRESS.md / checkpoints were left intact.
...
| Skill | Relationship |
|---|---|
| `/checkpointing` | Owns PROGRESS.md and `.Codex/checkpoints/`. Calls `/context-refresh` as its final "compact" step. context-refresh reads, but never rewrites, those artifacts. |
...
| `/design-tracker` | Operates only on `.Codex/docs/DESIGN.md`. No conflict. |
...
1. [ ] `@orchestra:template-boundary` present in AGENTS.md.
2. [ ] `@orchestra:repo-boundary` present in AGENTS.md.
...
7. [ ] No file in `.Codex/checkpoints/` is moved or deleted; `PROGRESS.md` is not regenerated.
...
10. [ ] Final recap reported to the user with new AGENTS.md line count.
