...
- Architecture is documented in `.Codex/docs/DESIGN.md`
...
- **AGENTS.md Zone C** (below `@orchestra:repo-boundary`) — current project context and decisions
- **`.Codex/docs/DESIGN.md`** — architecture and design decisions from the Architect
- **`.Codex/docs/research/`** — Researcher findings and library constraints
...
logs (`.Codex/logs/agent-teams/{team-name}/`) and research/design files line up
across phases.
...
- Architecture from .Codex/docs/DESIGN.md
- Library constraints from .Codex/docs/libraries/
...
1. **Implementer-{module}** for each module/workstream
   ...
   - .Codex/docs/DESIGN.md (architecture)
   - .Codex/docs/libraries/ (library constraints)
   ...
   When ALL your assigned tasks are complete, write a work log file to:
     .Codex/logs/agent-teams/{team-name}/{your-teammate-name}.md
...
2. **Tester** (optional but recommended)
   ...
   Read:
   - AGENTS.md, .Codex/docs/DESIGN.md
...
- **Cost awareness**: Each Teammate is an independent Codex instance (high token consumption)
